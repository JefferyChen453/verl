#!/usr/bin/env bash
# V3 4-GPU itml-1 fork: identical hyperparams to v3 but pinned to GPUs 0-3.
# See run_train_acrostics1139_z9_neg500_v3.sh for full diff history vs V2.
#
# Diffs from v3 8-GPU master:
#   1. CUDA_VISIBLE_DEVICES=0,1,2,3 (user authorized GPUs 0-4 on itml-1; use
#      4 for FSDP power-of-2 world size)
#   2. trainer.n_gpus_per_node=8 → 4
#   3. actor_rollout_ref.rollout.tensor_model_parallel_size=8 → 4 (must
#      divide world_size; recipe yaml default is 8)
#   4. experiment_name suffix: + "_itml1_4gpu" so wandb tracks separately
...
"${PY}" -m recipe.watermark_kd_ray.main \
    ...
    trainer.experiment_name=acrostics1139_z9_neg500_v3_s8_${DATE}_itml1_4gpu \
    2>&1 | tee "logs/acrostics1139_z9_neg500_v3_s8_${DATE}_itml1_4gpu.log"
