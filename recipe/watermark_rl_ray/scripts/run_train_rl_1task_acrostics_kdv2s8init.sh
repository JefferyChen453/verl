...
# Init from KD V2 s=8 epoch-3 (acrostics2906_neg500_v2_s8_*/global_step_1275/hf_model).
...
#
# V2-style memory optimizations are V2_GMU/OFFLOAD env-tunable so the
# orchestrator can OOM-retry by re-launching with different values without
# rewriting this script.
#
# Required env: V2_INIT_HF_PATH (KD V2 ep3 hf_model dir).
# Optional env: V2_GMU (default 0.5 — bumped 2026-05-03 after wandb analysis of
...
#                V2_PARAM_OFFLOAD (default false),
#               V2_OPTIM_OFFLOAD (default false), V2_REF_OFFLOAD (default false),
#               V2_EXP_SUFFIX (appended to experiment_name; useful for retry tags).
set -euo pipefail
...
ATTN_IMPL=${V2_ATTN_IMPL:-}      # if empty, don't set; let model default. e.g. "flash_attention_2"
SUFFIX=${V2_EXP_SUFFIX:-}

TS=$(date +%Y%m%d%H%M)
EXP_NAME="rl_1task_acrostics_kdv2s8init_grpo_${TS}${SUFFIX}"
