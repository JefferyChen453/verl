#!/usr/bin/env bash
# On-policy self-distillation with quality-filtered green bias.
# Self-distill teacher uses green ∩ ref_topk bias — only pushes green tokens
# that ref also considers quality-appropriate.
...
python -m recipe.watermark_onpolicy_ray.main \
    ...
    watermark.reverse_kl_biased_actor_actor_weight=1.0 \
    watermark.quality_green_topk=100 \
    ...
    trainer.experiment_name="onpolicy_strength_3.0_bsz_8__1.0reverse_sd_qg100_$(date +%Y%m%d%H%M)"
