#!/usr/bin/env bash
# On-policy training with L_green hinge + reverse quality anchor.
# Green signal: L_green with hinge target (saturates when avg green prob reaches target)
# Quality anchor: reverse KL(D_actor || D_ref) — prevents quality drift
# No self-distill KL — avoids unbounded push that collapses quality.
